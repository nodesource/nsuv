...
int ns_addrinfo::get(uv_loop_t* loop,
  ...
  if (cb == nullptr)
    return uv_getaddrinfo(loop, uv_req(), nullptr, node, service, hints);
...
int ns_addrinfo::get(uv_loop_t* loop,
  ...
  return uv_getaddrinfo(
      loop, uv_req(), &addrinfo_proxy_<decltype(cb)>, node, service, hints);
}
...
#define NSUV_LOOP_WATCHER_DEFINE(name)                                         \
  ...
  int ns_##name::start(void (*cb)(ns_##name*)) {                               \
    ...
    return uv_##name##_start(uv_handle(), &name##_proxy_<decltype(cb)>);       \
  }                                                                            \
                                                                               \
  template <typename D_T>                                                      \
  int ns_##name::start(void (*cb)(ns_##name*, D_T*), D_T* data) {              \
    ...
    if (cb == nullptr)                                                         \
      return uv_##name##_start(uv_handle(), nullptr);                          \
    return uv_##name##_start(uv_handle(), &name##_proxy_<decltype(cb), D_T>);  \
  }                                                                            \
                                                                               \
